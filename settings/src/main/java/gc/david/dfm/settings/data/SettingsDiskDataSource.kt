...
class SettingsDiskDataSource(private val context: Context) {

    fun getMeasureUnitPreference(): String {
        return getPreferences().getString(MEASURE_UNIT_KEY, MEASURE_EUROPEAN_UNIT_VALUE)!!
    }

    fun setMeasureUnitPreference(unit: String) {
        getPreferences().edit { putString(MEASURE_UNIT_KEY, unit) }
    }

    fun shouldShowElevationChart(): Boolean {
        return getPreferences().getBoolean(ELEVATION_CHART_KEY, true)
    }

    fun getAnimationPreference(): String? {
        return getPreferences().getString(ANIMATION_KEY, ANIMATION_CENTRE_VALUE)
    }
