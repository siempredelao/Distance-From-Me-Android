...
class MapStateMapper(private val context: Context) {
     ...
     */
    fun toMapUiState(model: DrawDistanceModel): MapUiState {
    ...
    private fun createMarkers(model: DrawDistanceModel): List<MarkerData> {
        ...
        return buildList {
            coordinates.forEachIndexed { index, coordinate ->
                ...
                val shouldShowMarker =
                    (index == 0 && (model.source == DrawDistanceModel.Source.DATABASE ||
    ...
    private fun createPolylines(model: DrawDistanceModel): List<PolylineData> {
        val coordinates = model.positionList
        ...
        val color = when (model.source) {
            DrawDistanceModel.Source.MANUAL -> LineColor.GREEN
            DrawDistanceModel.Source.DATABASE -> LineColor.YELLOW
        }
    ...
    private fun createCameraUpdate(model: DrawDistanceModel): CameraUpdate? {
